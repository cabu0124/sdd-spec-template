...
`specs/INDEX.md` lists every spec with its status and its consumers. It is the
product-facing catalogue, and it is regenerated by `/sdd-specify` and
`/sdd-status` rather than maintained by hand. An index that disagrees with the
specs is worse than no index: the spec headers win, always.

The `specs-index` job in `.github/workflows/specs-index.yml` enforces exactly
that on every pull request — a status the index does not share, a spec with no
row, a status outside the five, or a `plan.md` or `tasks.md` anywhere in the
